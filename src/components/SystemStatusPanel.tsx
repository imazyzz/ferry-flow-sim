...
export function SystemStatusPanel({
  ...
  const getStatusMessage = () => {
    if (systemStatus === "COLAPSO") {
      const maintenanceFerries = ferries.filter(
        (f) => f.state === "maintenance"
      );
  ...
  const avgUtilization =
    ferries.length > 0
      ? ferries.reduce((sum, f) => sum + calculateUtilization(f), 0) /
        ferries.length
      : 0;
  ...
  return (
    <Card className="p-4 space-y-4 bg-card border-2 border-primary/30">
      <div className="flex items-center justify-between">
        ...
        <Badge className={getStatusColor()}>{systemStatus}</Badge>
      </div>
