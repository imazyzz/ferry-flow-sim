    ...
    >
      ...
      <div className="mb-3">
        <div className="h-2 bg-background/30 rounded-full overflow-hidden">
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={{
              width: `${(ferry.vehicles.length / ferry.capacity) * 100}%`,
            }}
            transition={{ duration: 0.3 }}
          />
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {ferry.vehicles.length} / {ferry.capacity} veículos
        </div>
      </div>
      ...
      {ferry.state === "maintenance" && (
        <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-lg">
          <div
            className="absolute inset-0 bg-gradient-to-r from-transparent via-warning/20 to-transparent animate-pulse"
            style={{
              backgroundImage:
